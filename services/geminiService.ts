...
export const generatePortfolioFromResume = async (resumeImageBase64: string, resumeMimeType: string): Promise<CodeFile[]> => {
    try {
        ...
        const textPart = {
            text: "Analyze this resume image and extract all key information in a structured JSON format. Include name, contact info (email, phone, linkedin, website), a professional summary, a list of skills, work experience (company, role, dates, description bullet points), projects (name, description, technologies used), and education (institution, degree, dates). Ensure the output is only the JSON object.",
        };
