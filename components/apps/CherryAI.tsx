...
export const CherryAI: React.FC = () => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onerror = (event: any) => {
        ...
        if (event.error === 'no-speech') {
            setConversation(prev => [...prev, { key: Date.now(), role: 'ai', text: "Sorry, I didn't catch that. Please try again." }]);
        } else {
            setConversation(prev => [...prev, { key: Date.now(), role: 'ai', text: `A speech error occurred: ${event.error}.` }]);
        }
        setVoiceState('idle');
      };
  ...
  const executeCodeGeneration = async (promptToGenerate: string) => {
    ...
    if (updatedFiles.length > 0 && updatedFiles[0].fileName !== 'error.txt') {
        ...
        const errorMessage = { role: 'ai', text: `Sorry, I couldn't process that. The response was: ${updatedFiles[0]?.code || 'empty'}. Please try again.`, key: Date.now() + 1 };
  ...
  const handleSubmitPrompt = async () => {
    ...
    if(hasExistingCode) {
        ...
        setConversation(prev => [...prev, { key: Date.now() + 1, role: 'ai', text: `I've clarified your request to: "${enhancedPrompt}". Shall I proceed?` }]);
  ...
  const handleConfirmation = (confirm: boolean) => {
    if(confirm) {
        const proceedMessage = { role: 'user', text: "Yes, proceed.", key: Date.now() };
        setConversation(prev => [...prev, proceedMessage]);
        executeCodeGeneration(heldPrompt);
    } else {
        const cancelMessage = { role: 'user', text: "No, cancel.", key: Date.now() };
        const aiCancelMessage = { role: 'ai', text: "Okay, I've cancelled that request. What would you like to do instead?", key: Date.now()+1 };
 ...
 const handleOpenPreviewInNewTab = () => {
  ...
  const handleResumeFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setIsLoading(true);
      setConversation(prev => [...prev, { key: Date.now(), role: 'ai', text: 'Analyzing your resume to create a portfolio...' }]);
