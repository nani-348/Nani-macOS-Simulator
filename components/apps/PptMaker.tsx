...
export const PptMaker: React.FC = () => {
  ...
  const handleDownload = async () => {
     ...
     slides.forEach((slideData, index) => {
        ...
        slide.addText(slideData.title, { x: 0.5, y: 0.25, w: '90%', h: 1, fontSize: 28, bold: true, align: 'center', color: '363636' });
        
        // Fix: Map the array of strings to an array of {text: string} objects
        // to prevent a crash in the pptxgenjs library when creating bullet points.
        const textObjects = slideData.content.map(point => ({ text: point }));

        const textOptions: PptxGenJS.TextPropsOptions = {
            x: isTextLeft ? 0.5 : 5.2,
            y: 1.5,
            w: 4.5,
            h: 5.5,
            fontSize: 14,
