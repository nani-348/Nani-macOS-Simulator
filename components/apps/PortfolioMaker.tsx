...
import { generatePortfolioFromResume, generateOrModifyCode, type CodeFile } from '../../services/geminiService';
...
export const PortfolioMaker: React.FC = () => {
    ...
    const handleFileDrop = async (droppedFiles: FileList) => {
        ...
        if (file && file.type.startsWith('image/')) {
            ...
            setError('Please drop an image file (e.g., PNG, JPG) of your resume.');
        }
    };
    ...
    const renderUploadView = () => (
        <div 
            className="flex flex-col items-center justify-center h-full bg-gray-50 dark:bg-gray-800 p-8 text-center"
            onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
            onDragLeave={() => setIsDragging(false)}
            onDrop={(e) => { e.preventDefault(); setIsDragging(false); handleFileDrop(e.dataTransfer.files); }}
        >
            <div className={`w-full max-w-lg p-10 border-2 border-dashed rounded-xl transition-colors ${isDragging ? 'border-sky-500 bg-sky-50 dark:bg-sky-900/50' : 'border-gray-300 dark:border-gray-600'}`}>
                ...
                <p className="mt-2 text-gray-600 dark:text-gray-400">Drag and drop an image of your resume here, or click to upload.</p>
                <button 
                ...
                <input type="file" ref={fileInputRef} onChange={(e) => e.target.files && handleFileDrop(e.target.files)} className="hidden" accept="image/*" />
    ...
    const renderEditorView = () => (
        <div className="flex flex-col lg:flex-row h-full bg-[#1e1e1e] text-gray-300 font-mono">
            ...
            <div className="flex-grow flex flex-col relative">
                <div className="bg-[#252526] flex items-center justify-between border-b border-gray-700">
                ...
                <div className="flex-grow grid grid-cols-1 lg:grid-cols-2 overflow-hidden">
                    <pre className="w-full h-full p-4 overflow-auto text-sm bg-[#1e1e1e]"><code className="whitespace-pre-wrap">{activeFileContent}</code></pre>
